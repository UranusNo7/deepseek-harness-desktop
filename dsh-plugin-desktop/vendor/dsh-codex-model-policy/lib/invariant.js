...
const name = "codex-model-policy-invariant";
...
function validateSession(session, fail) {
	for (const event of session.events) if (event.type === "model-policy/fast") validateFast(event, fail);
}
/** Install validation for loaded and newly appended Fast-mode records. */
const install = Object.assign((ctx, fail) => {
	const sessions = ctx.get("sessions");
	if (typeof sessions.list === "function") for (const session of sessions.list()) validateSession(session, fail);
