 ...
 */
export function ModelSelect({ locked, available, directory, load, select, toggleFast, t }) {
    ...
    const chooseFast = () => {
        if (state.fast === null || (!state.fast.available && !state.fast.active))
            return;
        lastActionRef.current = 'select';
    ...
    return (_jsxs("div", { ref: rootRef, className: css.root, onKeyDown: onRootKeyDown, onBlur: onBlur, children: [_jsxs("button", { ref: triggerRef, type: "button", className: css.trigger, "aria-label": triggerAria, "aria-haspopup": "menu", "aria-expanded": open, "aria-controls": open ? `${id}-menu` : undefined, title: triggerLabel, disabled: locked, onClick: () => {
                    if (open) {
                        ...
                        show();
                                ...
                                : effortChoices.map(level => (_jsxs("button", { ref: itemRef(), type: "button", role: "menuitemradio", "aria-checked": effectiveEffort === level.effort, className: clsx(css.option, effectiveEffort === level.effort && css.selected), disabled: busy, onClick: () => { chooseEffort(level.effort); }, children: [_jsxs("span", { className: css.optionCopy, children: [_jsx("span", { className: css.modelName, children: level.label }), level.description !== undefined && (_jsx("span", { className: css.description, children: level.description }))] }), _jsx("span", { className: css.check, children: effectiveEffort === level.effort ? _jsx(IconCheckOutline16, {}) : null })] }, level.key))), (state.fast?.available === true || state.fast?.active === true) && (_jsxs("button", { ref: itemRef(), type: "button", role: "menuitemcheckbox", "aria-checked": state.fast.active, "aria-label": t(state.fast.active ? 'fast.ariaOn' : 'fast.ariaOff'), className: css.fast, disabled: busy, onClick: chooseFast, children: [_jsxs("span", { className: css.optionCopy, children: [_jsx("span", { className: css.modelName, children: t('fast.label') }), _jsx("span", { className: css.description, children: t(state.fast.active ? 'fast.stateOn' : 'fast.stateOff') })] }), _jsx("span", { className: css.check, children: state.fast.active ? _jsx(IconCheckOutline16, {}) : null })] }))] }))] })), toast !== null && (_jsx(Toast, { text: toast.text, icon: _jsx(IconWarningOutline16, {}), anchor: rootRef.current?.closest('[data-composer-card]') ?? null, onDone: () => { setToast(null); } }, toast.seq))] }));
}
