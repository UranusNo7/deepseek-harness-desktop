...
function profileOptions(profile, reasoning, apiKey, modelApi, serviceTier) {
    ...
    return {
        ...
        ...(serviceTier !== undefined && SERVICE_TIER_APIS.has(modelApi)
            ? { onPayload: (payload) => payloadWithServiceTier(payload, serviceTier) }
            : {}),
    ...
    async *stream(options) {
        ...
        try {
            ...
            if (serviceTier !== undefined && !SERVICE_TIER_APIS.has(model.api)) {
                throw new LlmError(`pi-ai model "${model.id}" uses API "${model.api}", which does not support service tier "${serviceTier}"`, 'UNSUPPORTED_OPTION');
            }
